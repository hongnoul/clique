...
[Overview](README.md) · [Vision](vision.md) · [Scheduler draft](ARCHITECTURE.MD) · [Research](research/README.md)
...
|---|---|---|---|
| Language/runtime | Python 3.12 + `uv` | Matches research docs; fast iteration. | Go (later, single-binary worker) |
...
| Model runtime | `llama.cpp` (`llama-server`) primary; Ollama adapter secondary | GGUF, Metal + CUDA prebuilt binaries, OpenAI-compatible endpoint. Matches ARCHITECTURE.MD. | vLLM (heavy for laptops), MLX (Mac-only) |
